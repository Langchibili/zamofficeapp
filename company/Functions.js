...
// Add these functions to your existing 📁frontend/Functions.js
...
export const registerCompany = async (companyData) => {
  try {
    // First, create the user in Strapi's users-permissions
    const userResponse = await fetch(`${api_url}/auth/local/register`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        ...
        confirmed: false, // Will be set to true after OTP verification
        // role: 3, // Company role ID - you'll need to set this in Strapi admin
      }),
    });

    if (!userResponse.ok) {
      const errorData = await userResponse.json();
      throw new Error(errorData.error?.message || 'User registration failed');
    }
  
    const userData = await userResponse.json();
     
    // Then create the company record linked to this user
    const companyPayload = {
      name: companyData.name,
      about: companyData.about,
      location: companyData.location,
      cost_per_page: companyData.cost_per_page,
      services: companyData.services,
      owner_name: companyData.ownerName,
      phone_number: companyData.phoneNumber,
      user: userData.user.id,
      float_balance: 0, // Start with 0 float
      status: 'pending_verification',
    }
    
    // Use the existing createCompany function with JWT
    const companyResult = await createCompany(companyPayload, userData.jwt);
    await updateUser({ company: companyResult.id, type:'company', confirmed: false }, userData.user.id, userData.jwt); // this is because at times updating the company type with the user relation doen't work, but vise versa
    saveJwt(userData.jwt) // to be used when updating user account after otp verification
    // Send OTP for email verification
    await sendOTP(companyData.email,"email")
    ...
    // Clean up: if user was created but company failed, delete the user
    if (error.message.includes('Company creation failed')) {
      // We would need a delete user function, but for now just log
      console.warn('Company creation failed after user was created. Manual cleanup may be needed.');
    }
...
export const loginCompany = async (email, password) => {
  try {
    ...
    const companies = await getCompanies(`filters[user][id][$eq]=${userData.user.id}&populate=*`);
    const company = companies && companies.length > 0 ? companies[0] : null;

    if (!company) {
      throw new Error('No company found for this user');
    }
    saveJwt(userData.jwt) // this logs a user in because the zm_company_jwt can be found locally
    return {
      user: userData.user,
      company: company,
      jwt: userData.jwt
...
export const verifyCompanyOtp = async (email, otp) => {
  try {
    // Use the existing verifyEmailOtp function
    const result = await verifyEmailOtp(email, otp);
    
    if (result && result.verified) {
      // Update user confirmed status
      const users = await getUsers(`filters[email][$eq]=${email}`);
      if (users && users.length > 0) {
        const user = users[0];
        await updateUser({ confirmed: true }, user.id);
      }
      
      // Update company status
      const companies = await getCompanies(`filters[user][email][$eq]=${email}`);
...
/**
 * Logout company - clear JWT token
 */
export const logoutCompany = () => {
  // Clear JWT token from localStorage or wherever it's stored
  localStorage.removeItem('zm_company_jwt');
 ...
 */
export const getCurrentCompany = async (jwt = null) => {
  try {
    ...
    // Get current user
    const response = await fetch(`${api_url}/users/me?populate=company`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
...
/**
 * Get queue for a specific company with prints populated
 * @param {number} companyId - The company ID
 * @param {object} options - Options object with params
 * @returns {Promise<object|null>} Queue object with prints or null
 */
export const getQueueForCompany = async (companyId, options = {}) => {
  try {
    const params = options.params || 'populate=prints,prints.client';
    
    const json = await zamFetch(
      `queues?filters[company][id][$eq]=${companyId}&${params}`
    );
    
    const queues = normalizeStrapiResponse(json, { returnArray: true });
    
    // Return first queue (should only be one per company)
    return queues && queues.length > 0 ? queues[0] : null;
  } catch (error) {
    console.error('Error fetching queue for company:', error);
    throw error;
  }
};

/**
 * Get all queued prints for a specific client at a specific company
 * @param {number} clientId - The client's database ID
 * @param {number} companyId - The company ID
 * @returns {Promise<Array>} Array of queued print objects
 */
export const getClientQueuedPrints = async (clientId, companyId) => {
  try {
    const json = await zamFetch(
      `prints?` +
      `filters[client][id][$eq]=${clientId}&` +
    ...
    console.error('Error fetching queued prints:', error);
    throw error;
  }
};
...
/* -------------------------
   UPDATED getCompanyByName to support print-based queue
   ------------------------- */
export const getCompanyByName = async (name, { exact = true, params = "" } = {}) => {
  if (!name) return null;
  const operator = exact ? "$eq" : "$containsi";
  
  // Default to populating queue with prints instead of clients
  const defaultParams = params || 'populate=queue,queue.prints,queue.prints.client';
  const q = `?filters[name][${operator}]=${name}&${defaultParams}`;
