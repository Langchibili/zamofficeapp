...
// import { Printer, Info, MapPin, Clock, FileText } from "lucide-react";
...
//         <CardDescription>
//           Live queue status and your position in line.
//         </CardDescription>
//       </CardHeader>
//       <CardContent>
...
//               <div className="font-medium">Total Prints in Queue</div>
//               <div className="text-2xl font-bold text-primary">{queueSize}</div>
//             </div>
//             <div className="text-sm">
//               <div className="font-medium">Your Prints</div>
//               <div className="text-2xl font-bold text-green-600">{myPrintPositions.length}</div>
//             </div>
//           </div>
...
//               <span>{myPrintPositions.length} of {queueSize}</span>
//             </div>
...
//                   You have {myPrintPositions.length} {myPrintPositions.length === 1 ? 'print' : 'prints'} in queue
//                 </div>
//               </div>
...
//             </div>
//           ) : (
//             <div className="p-3 rounded-md bg-gray-50 dark:bg-gray-950/20 border text-center">
//               <div className="text-sm text-muted-foreground">
//                 No prints in queue. Submit a print to see your position.
//               </div>
//             </div>
//           )}
//         </div>
...
//     {/* Recent Activity */}
//     {myQueuedPrints.length > 0 && (
//       <Card className="mt-4">
//         <CardHeader>
//           <CardTitle className="text-sm">Your Active Prints</CardTitle>
//         </CardHeader>
//         <CardContent>
//           <div className="space-y-3">
//             {myQueuedPrints.map(print => (
//               <div key={print.id} className="flex items-center justify-between p-3 border rounded-lg">
//                 <div className="flex items-center gap-3">
//                   <FileText className="h-4 w-4 text-muted-foreground" />
//                   <div>
//                     <div className="text-sm font-medium">
//                       {print.file_details?.filename || 'Document'}
//                     </div>
//                     <div className="text-xs text-muted-foreground">
//                       Submitted {new Date(print.createdAt).toLocaleTimeString()}
//                     </div>
//                   </div>
//                 </div>
//                 <Badge variant={
//                   print.state === 'printing' ? 'default' : 
//                   print.state === 'queued' ? 'secondary' : 'outline'
//                 }>
//                   {print.state}
//                 </Badge>
//               </div>
...
export default function CompanyPage() {
  ...
  const [creating, setCreating] = useState(false);
  const [showPrintInformation, setShowPrintInformation] = useState(false);
  const [printPreferences, setPrintPreferences] = useState(null);
  const [codeInput, setCodeInput] = useState("");
  const [onSiteByCode, setOnSiteByCode] = useState(false);
  const [isNearShop, setIsNearShop] = useState(false);
  const [nearbyAndFloat, setNearbyAndFloat] = useState(false);
  ...
  // Track multiple print positions
  const [myPrintPositions, setMyPrintPositions] = useState([]);
  const [myQueuedPrints, setMyQueuedPrints] = useState([]);
        ...
        if (queue && queue.prints && prints.length > 0) {
          const positions = prints.map(print => {
            ...
            return {
              ...
              position: position > 0 ? position : 999,
              orderId: print.order_id || `#${print.id}`,
              fileName: print.file_details?.filename || print.pdf_file?.name || 'Document'
            };
          }).filter(p => p.position < 999);
          
          setMyPrintPositions(positions);
          if (debugMode) console.log('[CompanyPage] Calculated positions:', positions);
        } else {
          setMyPrintPositions([]);
        }
      } catch (error) {
  ...
  // Socket event listeners for print-based queue
  useEffect(() => {
    if (!socket || !connected) return;
    ...
    const handleQueuePosition = (data) => {
      ...
      if (data.clientId === client?.clientId && data.prints && Array.isArray(data.prints)) {
        ...
        if (data.prints.length === 1) {
          ...
          const positions = data.prints.map((p) => p.position).join(', ');
          toast.info(`Your prints at positions: ${positions}`, {
            description: `${data.prints.length} prints in queue`,
          });
        }
    ...
    // NEW: Handle queue broadcast for all clients
    const handleQueueBroadcast = async (data) => {
      if (debugMode) console.log('[CompanyPage] Queue broadcast:', data);
      ...
      // Refetch full queue data
      await fetchQueue();
      await fetchMyPrints();
      
      if (debugMode) console.log('[CompanyPage] Processed queue broadcast');
    };

    const handlePrintQueued = async (data) => {
      if (debugMode) console.log('[CompanyPage] Print queued event:', data);
      ...
      // Refetch queue for ALL clients when ANY print is queued
      await fetchQueue();
      await fetchMyPrints();
    };

    const handlePrintScheduled = (data) => {
      if (data.printId === print?.id) {
        if (debugMode) console.log('[CompanyPage] Your print was scheduled!');
        setPrintStatus('scheduled');
    ...
    const handlePrintPrinting = async (data) => {
      if (debugMode) console.log('[CompanyPage] Print printing event:', data);
    ...
    const handlePrintCompleted = async (data) => {
      if (debugMode) console.log('[CompanyPage] Print completed event:', data);
    ...
    const handlePrintCanceled = async (data) => {
      if (debugMode) console.log('[CompanyPage] Print canceled event:', data);
    ...
    const handleCompanyUnavailable = (data) => {
      if (data.companyId === company?.id) {
        if (debugMode) console.log('[CompanyPage] Company became unavailable');
        toast.warning('Company unavailable', {
          description: data.reason === 'float_depleted' 
    ...
    const handleCompanyFloatToppedUp = (data) => {
      if (data.companyId === company?.id) {
        if (debugMode) console.log('[CompanyPage] Company float topped up');
        toast.success('Company float replenished', {
          description: 'You can now submit prints immediately',
    ...
    socket.on('queue:position', handleQueuePosition);
    socket.on('queue:updated', handleQueueUpdated);
    socket.on('queue:broadcast', handleQueueBroadcast); // NEW: Broadcast listener
    socket.on('print:queued', handlePrintQueued);
    socket.on('print:scheduled', handlePrintScheduled);
    socket.on('print:printing', handlePrintPrinting);
    socket.on('print:completed', handlePrintCompleted);
    ...
    return () => {
      ...
      socket.off('queue:position', handleQueuePosition);
      socket.off('queue:updated', handleQueueUpdated);
      socket.off('queue:broadcast', handleQueueBroadcast); // NEW: Cleanup broadcast
      socket.off('print:queued', handlePrintQueued);
      socket.off('print:scheduled', handlePrintScheduled);
      socket.off('print:printing', handlePrintPrinting);
      socket.off('print:completed', handlePrintCompleted);
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (queueData && queueData.prints && myPrints.length > 0) {
          const positions = myPrints.map(myPrint => {
            const position = queueData.prints.findIndex((p) => p.id === myPrint.id) + 1;
            return {
              printId: myPrint.id,
              position: position > 0 ? position : 999,
              orderId: myPrint.order_id || `#${myPrint.id}`,
              fileName: myPrint.file_details?.filename || myPrint.pdf_file?.name || 'Document'
            };
          }).filter(p => p.position < 999);
  ...
  // Render multiple print positions
  const renderQueuePositions = () => {
    if (!myPrintPositions || myPrintPositions.length === 0) {
      return (
        <div className="mt-4 text-sm text-muted-foreground">
  ...
  // Calculate estimated wait from print positions
  const estWait = myPrintPositions.length > 0
    ? Math.min(...myPrintPositions.map(p => p.position)) * 5
    : queueSize * 2;
                  ...
                  {myQueuedPrints.map(print => (
                    <div key={print.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-medium">
                            {print.file_details?.filename || print.pdf_file?.name || 'Document'}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Submitted {new Date(print.createdAt).toLocaleTimeString()}
                          </div>
